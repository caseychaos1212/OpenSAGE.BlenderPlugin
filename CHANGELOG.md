...
* Merge upstream Blender 5.1/5.2 compatibility changes, including material, UV,
  vertex-color, and XML API updates, while retaining the OpenW3D Renegade tools.
* Add extension packaging with OpenW3D metadata and relative imports throughout
  the add-on; keep the unmaintained updater removed.
* Apply the compatibility helpers to the OpenW3D material bridge and share
  action-curve access with the existing animation compatibility layer.
* Update CI downloads and add Blender 5.2 to the test matrix.
